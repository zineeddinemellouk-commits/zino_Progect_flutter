...
class OptionsUtil {
    ...
    _getKnownOptions(options, context) {
        const knownOptions = commonFa1cc002_node.ObjectValue.empty();
        ...
        for (const knownOptionKey in this.optionDefinitions) {
            if (this.optionDefinitions.hasOwnProperty(knownOptionKey)) {
                ...
                if (knownOptionKey in options) {
                    ...
                    if (optionDefinition.nestedOptions && commonFa1cc002_node.isPlainObject(optionValue)) {
                        ...
                        protoValue = commonFa1cc002_node.parseData(optionValue, context) ?? undefined;
                    }
                    if (protoValue) {
                        knownOptions.set(commonFa1cc002_node.FieldPath$1.fromServerFormat(optionDefinition.serverName), protoValue);
                    }
                }
            }
        }
    ...
    getOptionsProto(context, knownOptions, optionsOverride) {
        ...
        if (optionsOverride) {
            const optionsMap = new Map(commonFa1cc002_node.mapToArray(optionsOverride, (value, key) => [
                commonFa1cc002_node.FieldPath$1.fromServerFormat(key),
                value !== undefined ? commonFa1cc002_node.parseData(value, context) : null
            ]));
...
function isIMapValue(obj) {
    ...
    if ('fields' in obj && (obj.fields === null || commonFa1cc002_node.isPlainObject(obj.fields))) {
        return true;
    }
    return false;
}
...
function valueToDefaultExpr$1(value) {
    ...
    else if (commonFa1cc002_node.isPlainObject(value)) {
        result = _map(value);
    }
...
function vectorToExpr$1(value) {
    ...
    else if (value instanceof commonFa1cc002_node.VectorValue) {
        return constant(value);
    }
    else if (Array.isArray(value)) {
        return constant(commonFa1cc002_node.vector(value));
    }
    else {
...
function fieldOrExpression$1(value) {
    if (commonFa1cc002_node.isString(value)) {
...
class Expression {
    ...
    asBoolean() {
        if (this instanceof BooleanExpression) {
            ...
            throw new commonFa1cc002_node.FirestoreError('invalid-argument', `Conversion of type ${typeof this} to BooleanExpression not supported.`);
        }
    }
    subtract(subtrahend) {
...
function _field(nameOrPath, methodName) {
    if (typeof nameOrPath === 'string') {
        if (commonFa1cc002_node.DOCUMENT_KEY_NAME === nameOrPath) {
            return new Field(commonFa1cc002_node.documentId()._internalPath, methodName);
        }
        return new Field(commonFa1cc002_node.fieldPathFromArgument('field', nameOrPath), methodName);
    }
    else {
...
class Constant extends Expression {
     ...
     */
    _toProto(_) {
        commonFa1cc002_node.hardAssert(this._protoValue !== undefined, 0x00ed);
    ...
    _readUserData(context) {
        ...
        if (isFirestoreValue(this._protoValue)) {
            ...
            this._protoValue = commonFa1cc002_node.parseData(this.value, context);
        }
    }
}
...
class MapValue extends Expression {
    ...
    _toProto(serializer) {
        return commonFa1cc002_node.toMapValue(serializer, this.plainObject);
    }
}
/**
 *
...
function timestampTruncate(fieldNameOrExpression, granularity, timezone) {
    const internalGranularity = commonFa1cc002_node.isString(granularity)
...
class PipelineValueExpression extends Expression {
    ...
    _toProto(jsonProtoSerializer) {
        return commonFa1cc002_node.toPipelineValue(this.pipeline._toProto(jsonProtoSerializer));
    }
    /**
     * @internal
     */
...
class Ordering {
    ...
    _toProto(serializer) {
        return {
            mapValue: {
                fields: {
                    direction: commonFa1cc002_node.toStringValue(this.direction),
...
function isSelectable(val) {
    const candidate = val;
    return (candidate.selectable && commonFa1cc002_node.isString(candidate.alias) && isExpr(candidate.expr));
}
...
function isAliasedAggregate(val) {
    const candidate = val;
    return (commonFa1cc002_node.isString(candidate.alias) &&
...
function toPipelineBooleanExpr(f) {
    if (f instanceof commonFa1cc002_node.FieldFilter) {
        ...
        switch (f.op) {
            ...
            default:
                commonFa1cc002_node.fail(0x9047);
        }
    }
    else if (f instanceof commonFa1cc002_node.CompositeFilter) {
        switch (f.op) {
            ...
            default:
                commonFa1cc002_node.fail(0x89ea);
        }
    }
...
function toPipeline(query, db) {
    let pipeline;
    if (commonFa1cc002_node.isCollectionGroupQuery(query)) {
        pipeline = db.pipeline().collectionGroup(query.collectionGroup);
    }
    else if (commonFa1cc002_node.isDocumentQuery(query)) {
        pipeline = db.pipeline().documents([commonFa1cc002_node.doc(db, query.path.canonicalString())]);
    }
    else {
    ...
    // orders
    const orders = commonFa1cc002_node.queryNormalizedOrderBy(query);
...
function selectablesToObject(selectables) {
    ...
    for (const selectable of selectables) {
        ...
        if (typeof selectable === 'string') {
            ...
            commonFa1cc002_node.fail(0x5319, { selectable });
        }
        if (result[alias] !== undefined) {
            throw new commonFa1cc002_node.FirestoreError('invalid-argument', `Duplicate alias or field '${alias}'`);
        }
...
function aliasedAggregateToMap(aliasedAggregatees) {
    return aliasedAggregatees.reduce((map, selectable) => {
        if (map.get(selectable.alias) !== undefined) {
            throw new commonFa1cc002_node.FirestoreError('invalid-argument', `Duplicate alias or field '${selectable.alias}'`);
        }
...
function vectorToExpr(value) {
    if (value instanceof Expression) {
        ...
        const result = constant(commonFa1cc002_node.vector(value));
        return result;
    }
    else {
        throw new Error('Unsupported value: ' + typeof value);
...
class AddFields extends Stage {
    ...
    _toProto(serializer) {
        return {
            ...
            args: [commonFa1cc002_node.toMapValue(serializer, this.fields)]
        };
    }
    _readUserData(context) {
...
class Define extends Stage {
    ...
    _toProto(serializer) {
        return {
            ...super._toProto(serializer),
            args: [commonFa1cc002_node.toMapValue(serializer, this.aliasedExpressions)]
        };
    }
    _readUserData(context) {
        super._readUserData(context);
...
class Aggregate extends Stage {
    ...
    _toProto(serializer) {
        return {
            ...super._toProto(serializer),
            args: [
                commonFa1cc002_node.toMapValue(serializer, this.accumulators),
                commonFa1cc002_node.toMapValue(serializer, this.groups)
            ]
        };
    }
    _readUserData(context) {
...
class Distinct extends Stage {
    ...
    _toProto(serializer) {
        return {
            ...super._toProto(serializer),
            args: [commonFa1cc002_node.toMapValue(serializer, this.groups)]
        };
    }
    _readUserData(context) {
        super._readUserData(context);
    ...
    _toProto(serializer) {
        return {
            ...
            args: [
                ...
                commonFa1cc002_node.toStringValue(this.distanceMeasure)
            ]
        };
    }
    _readUserData(context) {
...
class Limit extends Stage {
    ...
    constructor(limit, options) {
        commonFa1cc002_node.hardAssert(!isNaN(limit) && limit !== Infinity && limit !== -Infinity, 0x882c);
        super(options);
        this.limit = limit;
    }
    /**
    ...
    _toProto(serializer) {
        return {
            ...super._toProto(serializer),
            args: [commonFa1cc002_node.toNumber(serializer, this.limit)]
        };
    }
}
class Offset extends Stage {
    ...
    _toProto(serializer) {
        return {
            ...super._toProto(serializer),
            args: [commonFa1cc002_node.toNumber(serializer, this.offset)]
        };
    }
}
class Select extends Stage {
    ...
    _toProto(serializer) {
        return {
            ...super._toProto(serializer),
            args: [commonFa1cc002_node.toMapValue(serializer, this.selections)]
        };
    }
    _readUserData(context) {
        super._readUserData(context);
...
class Sample extends Stage {
    ...
    _toProto(serializer) {
        return {
            ...super._toProto(serializer),
            args: [commonFa1cc002_node.toNumber(serializer, this.rate), commonFa1cc002_node.toStringValue(this.mode)]
        };
    }
    _readUserData(context) {
        super._readUserData(context);
...
class Union extends Stage {
    ...
    _toProto(serializer) {
        return {
            ...super._toProto(serializer),
            args: [commonFa1cc002_node.toPipelineValue(this.other._toProto(serializer))]
        };
    }
    _readUserData(context) {
...
class Replace extends Stage {
    ...
    _toProto(serializer) {
        return {
            ...super._toProto(serializer),
            args: [this.map._toProto(serializer), commonFa1cc002_node.toStringValue(Replace.MODE)]
        };
    }
    _readUserData(context) {
        super._readUserData(context);
...
function readUserDataHelper(expressionMap, context) {
    if (commonFa1cc002_node.isUserData(expressionMap)) {
...
class Pipeline {
    ...
    removeFields(fieldValueOrOptions, ...additionalFields) {
        // Process argument union(s) from method overloads
        const options = isField(fieldValueOrOptions) || commonFa1cc002_node.isString(fieldValueOrOptions)
            ? {}
            : fieldValueOrOptions;
        const fields = isField(fieldValueOrOptions) || commonFa1cc002_node.isString(fieldValueOrOptions)
            ? [fieldValueOrOptions, ...additionalFields]
            : fieldValueOrOptions.fields;
        // Convert user land convenience types to internal types
        const convertedFields = fields.map(f => commonFa1cc002_node.isString(f) ? field(f) : f);
        // Create stage object
    ...
    select(selectionOrOptions, ...additionalSelections) {
        // Process argument union(s) from method overloads
        const options = isSelectable(selectionOrOptions) || commonFa1cc002_node.isString(selectionOrOptions)
            ? {}
            : selectionOrOptions;
        const selections = isSelectable(selectionOrOptions) || commonFa1cc002_node.isString(selectionOrOptions)
    ...
    offset(offsetOrOptions) {
        ...
        let options;
        let offset;
        if (commonFa1cc002_node.isNumber(offsetOrOptions)) {
            options = {};
            offset = offsetOrOptions;
        }
        else {
    ...
    limit(limitOrOptions) {
        // Process argument union(s) from method overloads
        const options = commonFa1cc002_node.isNumber(limitOrOptions) ? {} : limitOrOptions;
        const limit = commonFa1cc002_node.isNumber(limitOrOptions)
            ? limitOrOptions
    ...
    distinct(groupOrOptions, ...additionalGroups) {
        // Process argument union(s) from method overloads
        const options = commonFa1cc002_node.isString(groupOrOptions) || isSelectable(groupOrOptions)
            ? {}
            : groupOrOptions;
        const groups = commonFa1cc002_node.isString(groupOrOptions) || isSelectable(groupOrOptions)
    ...
    replaceWith(valueOrOptions) {
        // Process argument union(s) from method overloads
        const options = commonFa1cc002_node.isString(valueOrOptions) || isExpr(valueOrOptions) ? {} : valueOrOptions;
        const fieldNameOrExpr = commonFa1cc002_node.isString(valueOrOptions) || isExpr(valueOrOptions)
            ? valueOrOptions
            : valueOrOptions.map;
        // Convert user land convenience types to internal types
    ...
    sample(documentsOrOptions) {
        // Process argument union(s) from method overloads
        const options = commonFa1cc002_node.isNumber(documentsOrOptions) ? {} : documentsOrOptions;
        let rate;
        let mode;
        if (commonFa1cc002_node.isNumber(documentsOrOptions)) {
            rate = documentsOrOptions;
            mode = 'documents';
        }
        else if (commonFa1cc002_node.isNumber(documentsOrOptions.documents)) {
    ...
    unnest(selectableOrOptions, indexField) {
        ...
        if (commonFa1cc002_node.isString(indexFieldName)) {
...
class PipelineSource {
    ...
    collection(collectionOrOptions) {
        // Process argument union(s) from method overloads
        const options = commonFa1cc002_node.isString(collectionOrOptions) ||
            commonFa1cc002_node.isCollectionReference(collectionOrOptions)
            ? {}
            : collectionOrOptions;
        const collectionRefOrString = commonFa1cc002_node.isString(collectionOrOptions) ||
            commonFa1cc002_node.isCollectionReference(collectionOrOptions)
            ? collectionOrOptions
            : collectionOrOptions.collection;
        // Validate that a user provided reference is for the same Firestore DB
        if (commonFa1cc002_node.isCollectionReference(collectionRefOrString)) {
            this._validateReference(collectionRefOrString);
        }
        // Convert user land convenience types to internal types
        const normalizedCollection = commonFa1cc002_node.isString(collectionRefOrString)
    ...
    collectionGroup(collectionIdOrOptions) {
        ...
        let collectionId;
        let options;
        if (commonFa1cc002_node.isString(collectionIdOrOptions)) {
    ...
    documents(docsOrOptions) {
        ...
        docs
            .filter(v => v instanceof commonFa1cc002_node.DocumentReference)
            .forEach(dr => this._validateReference(dr));
        // Convert user land convenience types to internal types
        const normalizedDocs = docs.map(doc => commonFa1cc002_node.isString(doc) ? doc : doc.path);
        // Create stage object
    ...
    _validateReference(reference) {
        ...
        if (!refDbId.isEqual(this.databaseId)) {
            throw new commonFa1cc002_node.FirestoreError(commonFa1cc002_node.Code.INVALID_ARGUMENT, `Invalid ${reference instanceof commonFa1cc002_node.CollectionReference
                ? 'CollectionReference'
                : 'DocumentReference'}. ` +
...
function subcollection(pathOrOptions) {
    ...
    let path;
    let options;
    if (commonFa1cc002_node.isString(pathOrOptions)) {
        path = pathOrOptions;
        options = {};
    }
    else {
...
class PipelineResult {
    ...
    get(fieldPath) {
        ...
        const value = this._fields.field(commonFa1cc002_node.fieldPathFromArgument('DocumentSnapshot.get', fieldPath));
        if (value !== null) {
...
function execute(pipeline) {
    if (!pipeline._db) {
        return Promise.reject(new commonFa1cc002_node.FirestoreError(commonFa1cc002_node.Code.FAILED_PRECONDITION, 'This pipeline was created without a database (e.g., as a subcollection pipeline) and cannot be executed directly. It can only be used as part of another pipeline.'));
    }
    const datastore = commonFa1cc002_node.getDatastore(pipeline._db);
    const firestore = commonFa1cc002_node.cast(pipeline._db, commonFa1cc002_node.Firestore);
    const userDataReader = commonFa1cc002_node.newUserDataReader(firestore);
    ...
    return commonFa1cc002_node.invokeExecutePipeline(datastore, structuredPipeline).then(result => {
        ...
        const docs = result
            ...
            ? new commonFa1cc002_node.DocumentReference(firestore, null, element.key)
 ...
 * ```
 */
commonFa1cc002_node.Firestore.prototype.pipeline = function () {
